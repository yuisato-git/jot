    ...
    <div>
        ...
        <div v-else="loading">
            <div class="flex justify-between">
                ...
                <div class="relative">
                    <router-link :to="'/contacts/' + contact.contact_id + '/edit'" class="px-4 py-2 rounded text-sm
                text-green-500 border border-green-500 font-bold mr-2">Edit</router-link>
                    ...
                    <div v-if="modal" class="absolute bg-blue-900 text-white rounded-lg z-20 p-8 w-64 right-0 mt-2 mr-6">
                        ...
                        <div class="flex items-center mt-6 justify-end">
                            <button class="text-white pr-4 focus:outline-none" @click="modal =  !modal">Cancel</button>
                            <button class="px-4 py-2 bg-red-500 rounded text-sm font-bold" @click="destroy()">Delete</button>
                        </div>
                    </div>
                </div>
                <div v-if="modal" class="bg-black opacity-25 absolute inset-0 z-10" @click="modal = !modal"></div>
            </div>
